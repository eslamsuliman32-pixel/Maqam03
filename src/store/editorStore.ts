...
export const useEditorStore = create<EditorState>()(
  persist(
    (set) => ({
      ...
      addFlowMarker: (marker) =>
        set((state) => ({
          flowMarkers: [
            ...state.flowMarkers.filter((m) => m.cellId !== marker.cellId),
            marker,
          ],
        })),
      updateFlowMarkerText: (cellId, text) =>
        set((state) => ({
          flowMarkers: state.flowMarkers.map((m) =>
            m.cellId === cellId ? { ...m, text } : m,
          ),
        })),
      removeFlowMarker: (cellId) =>
        set((state) => ({
