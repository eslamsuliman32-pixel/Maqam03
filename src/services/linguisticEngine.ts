...
export class LinguisticEngine {
  ...
  generateWordCloud(
    ...
    repositoryBars.forEach((barText) => {
      ...
      if (phoneticScore > 0.4 || semanticMatch) {
        cloud.push({
          ...
          type: "repository_bar",
          connectionLabel: semanticMatch
            ? "رابط دلالي من مستودعك"
            : "رابط صوتي من مستودعك",
    ...
    this.idiomsDB.forEach((idiom) => {
      const isRelevant =
        words.some((w) => idiom.tags.includes(w)) ||
    ...
    cloud.forEach((item) => {
      if (
        ...
        uniqueMap.get(item.word)!.phonemicMatchScore < item.phonemicMatchScore
      ) {
    ...
    return Array.from(uniqueMap.values()).sort(
      (a, b) =>
        b.phonemicMatchScore - a.phonemicMatchScore || Math.random() - 0.5,
    );
  }
   ...
   */
  decomposeToPianoKeys(
    phrase: string,
  ): { syllable: string; weight: number; index: number }[] {
    ...
    words.forEach((word, wordIdx) => {
      ...
      parts.forEach((part) => {
        ...
        keys.push({
          ...
          weight: isStressed
            ? 1.5
            : moraEngine.analyze(part).totalMorae > 1
              ? 1.2
              : 0.8,
          index: keys.length,
        });
  ...
  tidyRhythm(
    syllables: string[],
  ): { suggestion: string; surpriseFactor: boolean }[] {
   ...
   */
  bendSyllable(word: string): {
    suggestedBend: string;
    surpriseFactor: boolean;
  } {
  ...
  applyAcousticEngineering(
    ...
    switch (technique) {
      case "derivation":
        ...
        if (w.includes("دقق"))
          return ["دقة", "مدقق", "دقيق", "تدقيق", "استدقاق", "دقات"];
        if (w.includes("كسر"))
          return ["مكسور", "انكسار", "كسره", "تكسير", "تكتيك مستتر"];
      ...
      case "suffix":
        ...
        return [
          `كلمة تنتهي بـ (${suffix})`,
          `مصطلح أنمي/إنجليزي ينتهي بـ (${suffix})`,
          `ماركة تجارية تنتهي بـ (${suffix})`,
        ];

      case "deconstruct":
        if (w === "قنبلة")
          return ["قنبلة من دون النون (قبلة)", "فنون", "مجنون"];
        if (w.length > 3) {
          return [
      ...
      case "cross_lang":
        if (w.includes("أناناسة") || w.toLowerCase().includes("juice"))
          return ["I got the juice", "أناناسة", "شفت كيف السلاسة؟", "رئاسة"];
        if (w.includes("ديل") || w.toLowerCase().includes("deal"))
          return ["Deal", "تقيل", "Real", "طويل"];
        return [
