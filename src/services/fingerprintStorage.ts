...
const REVERSE_FOOT_MAP: Record<number, MetricalFoot> = Object.fromEntries(
  Object.entries(FOOT_MAP).map(([k, v]) => [v, k as MetricalFoot]),
);
...
function decodeFingerprint(data: Uint8Array): PhonemicFingerprint {
  const bits = Array.from(data.slice(0, data.length - 2)).map((b) =>
    b === 1 ? "[!]" : "[.]",
  ) as AccentBit[];
...
export class FingerprintStorage {
  ...
  async init(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        store.createIndex("fingerprintCode", "fingerprintCode", {
          unique: false,
        });
      };
  ...
  async get(hash: string): Promise<PhonemicFingerprint | null> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () =>
        resolve(request.result ? decodeFingerprint(request.result.data) : null);
  ...
  async findByFoot(
    foot: MetricalFoot,
  ): Promise<{ hash: string; fingerprint: PhonemicFingerprint }[]> {
    if (!this.db) await this.init();
    return new Promise((resolve, reject) => {
      const transaction = this.db!.transaction(STORE_NAME, "readonly");
      const store = transaction.objectStore(STORE_NAME);
    ...
    return new Promise((resolve, reject) => {
      ...
      store.openCursor().onsuccess = (event) => {
        ...
        if (cursor) {
          ...
          const bitsBinary = fingerprint.bits
            .map((b) => (b === "[!]" ? "1" : "0"))
            .join("");
