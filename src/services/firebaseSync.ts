...
const localKey = (namespace: string, name: string) =>
  ...
  try {
    ...
    for (let pos = 0; pos < bytes.length; pos += chunkSize) {
      binary += String.fromCharCode.apply(
        null,
        Array.from(bytes.subarray(pos, pos + chunkSize)),
      );
    }
    return btoa(binary);
  } catch (err) {
...
const decompressPayload = async (
  base64Str: string,
  isCompressed: boolean,
): Promise<string> => {
  if (!isCompressed || !canUseWindow || !window.DecompressionStream)
    return base64Str;
  try {
...
const extractPayloadFromRecord = async (
  record: any,
): Promise<string | null> => {
...
const createSnapshot = async (
  ...
  const sDoc = snapshotDoc(uid, namespace, name, id);
  await setDoc(
    sDoc,
    {
    ...
    handleFirestoreError(error, OperationType.WRITE, sDoc.path);
  });
};
...
const pushToCloud = async (
  ...
  try {
    ...
    const shouldSnapshot =
      forceSnapshot ||
      !currentPayload ||
      Math.abs(payload.length - currentPayload.length) >=
        options.snapshotMinSizeDelta ||
      Math.abs(payload.length - (currentPayload?.length ?? 0)) /
        Math.max(currentPayload?.length ?? 1, 1) >=
        options.snapshotThresholdRatio;
    ...
    const finalPayload = isCompressed
...
export const createCloudSyncStorage = (options?: SyncOptions): StateStorage => {
  ...
  const flushAll = async (forceSnapshot = false) => {
    const promises = Array.from(pendingValues.keys()).map((name) =>
      flush(name, forceSnapshot),
    );
  ...
  return {
    ...
    removeItem: async (name: string): Promise<void> => {
      ...
      try {
        const payload = "";
        const tDoc = stateDoc(user.uid, resolved.namespace, name);
        await setDoc(
          tDoc,
          {
          ...
          handleFirestoreError(error, OperationType.WRITE, tDoc.path);
        });
      } catch (error) {
