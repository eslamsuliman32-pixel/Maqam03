...
export class MoraEngine {
  ...
  private readonly MUTAWASSITA: Set<string> = new Set([
    "ل",
    "ن",
    "ر",
    "م",
  ...
  private readonly HULQI: Set<string> = new Set([
    "ع",
    "ح",
    "غ",
    "خ",
  ...
  private readonly ARTICULATION_ZONES: Record<
    string,
    PhoneticProperty["articulationZone"]
  > = {
    ب: "labial",
    م: "labial",
    و: "labial",
  ...
  getSlantRhymeGroup(phoneme: string): string {
    ...
    if (["د", "ت", "ط", "ض"].includes(lastChar))
      return "نطعية/مائلة (د، ت، ط، ض)";
    if (["ب", "م", "ف"].includes(lastChar)) return "شفوية/مائلة (ب، م، ف)";
    if (["ق", "ك", "غ", "خ"].includes(lastChar))
      return "لهوية/مائلة (ق، ك، غ، خ)";
    if (["س", "ص", "ز", "ش"].includes(lastChar))
      return "أسلية/مائلة (س، ص، ز، ش)";
  ...
  getPhoneticTrait(text: string): string {
    ...
    for (const char of cleanText) {
      ...
      else if (this.phoneticTraits["متوسطة (توازن)"].includes(char))
        mutawassita++;
      else rakhwa++;
    }
  ...
  getPhoneticProperties(char: string): PhoneticProperty {
    return {
      ...
      articulationZone: this.ARTICULATION_ZONES[char] ?? "alveolar",
    };
  }
  ...
  analyzeAcousticResonance(text: string): AcousticResonanceProfile {
    ...
    const chars = Array.from(cleanText).filter((c) =>
      /[\u0600-\u06FF]/.test(c),
    );
    ...
    const qalqalaIntensity = Math.min(
      100,
      Math.round((qalqalaCount / consonantCount) * 100 * 1.6),
    );
    const rakhwaIndex = Math.min(
      100,
      Math.round((rakhwaCount / consonantCount) * 100),
    );
    const shiddaScore = Math.min(
      100,
      Math.round((shiddaCount / consonantCount) * 100 * 1.2),
    );
    const safirDensity = Math.min(
      100,
      Math.round((safirCount / consonantCount) * 100 * 2.0),
    );
    const hulqiDepth = Math.min(
      100,
      Math.round((hulqiCount / consonantCount) * 100 * 1.3),
    );
    ...
    const overallResonance = Math.min(
      100,
      Math.round(
  ...
  analyzePhonemeCharacterization(text: string): PhonemeCharacterization {
    ...
    if (hardness > softness && hardness > nasality && hardness > 25)
      dominantTrait = "hard";
    else if (softness > hardness && softness > nasality && softness > 25)
      dominantTrait = "soft";
    else if (nasality > hardness && nasality > softness && nasality > 15)
      dominantTrait = "nasal";
    ...
    const descMap = {
      ...
      nasal:
        "طابع أنفي (Nasality) يخلق رنيناً متصلاً (Ghunna) يساعد على ربط الكلمات ببعضها.",
  ...
  private classifyAcousticCharacter(
    ...
    return (
      Object.entries(scores) as [DominantAcousticCharacter, number][]
  ...
  private computeSonicWeight(units: MoraUnit[], totalMorae: number): number {
    ...
    const avgMultiplier =
      units.length > 0
        ? units.reduce((sum, u) => sum + multipliers[u.phonemeClass], 0) /
          units.length
        : 1;
    ...
    return Math.min(
      100,
      Math.round(totalMorae * avgMultiplier * densityMultiplier * 5),
    );
  }
  ...
  async processBatchAsync(
    ...
    return this.dispatchToWorker<MoraProfile[]>("processBatch", [
      texts,
      dialect,
    ]);
  }
  ...
  async analyzeAcousticResonanceAsync(
    text: string,
  ): Promise<AcousticResonanceProfile> {
    return this.dispatchToWorker<AcousticResonanceProfile>(
  ...
  async batchAcousticResonanceAsync(
    texts: string[],
  ): Promise<AcousticResonanceProfile[]> {
    return this.dispatchToWorker<AcousticResonanceProfile[]>(
      "batchAcousticResonance",
      [texts],
    );
  }
}
