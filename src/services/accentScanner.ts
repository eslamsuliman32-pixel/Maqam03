...
class AccentScanner {
  ...
  public async buildFingerprint(
    text: string,
    dialect: Dialect = "standard",
  ): Promise<PhonemicFingerprint> {
    ...
    const profile = moraEngine.analyze(
      text,
      dialect === "standard" ? "fusha" : (dialect as any),
    );
    const rhyme = moraEngine.extractCorePhoneme(text);
    const code = this.generateFingerprintCode(
      text,
      bits,
      profile.totalMorae,
      rhyme,
  ...
  public computeSyncopeScore(bits: AccentBit[]): number {
    ...
    const offBeatStresses = bits.filter(
      (b, i) => b === "[!]" && i % 2 === 1,
    ).length;
  ...
  public detectFlowSwitches(
    ...
    for (let i = 0; i < bits.length - windowSize; i += 2) {
      ...
      const prevDensity =
        prevWindow.filter((b) => b === "[!]").length / prevWindow.length;
      const nextDensity =
        nextWindow.filter((b) => b === "[!]").length / nextWindow.length;
      const delta = nextDensity - prevDensity;

      if (Math.abs(delta) < threshold) continue;

      const prevPattern = prevWindow
        .slice(-3)
        .join("")
        .replace(/[\[\]]/g, "");
      const nextPattern = nextWindow
        .slice(0, 3)
        .join("")
        .replace(/[\[\]]/g, "");
  ...
  public analyzeDominantGrid(bits: AccentBit[]): RhythmicGrid {
    ...
    const best = (Object.entries(periods) as [string, number][]).reduce(
      (a, b) => (b[1] > a[1] ? b : a),
    );
  ...
  public analyzePolyrhythm(
    ...
    const tension = Math.min(
      100,
      Math.round(index * 0.55 + syncopeScore * 0.45),
    );
  ...
  public async buildFullFingerprint(
    ...
    const extendedCode =
      this.generateFingerprintCode(text, bits, profile.totalMorae, rhyme) +
  ...
  async analyzePolyrhythmAsync(
    text: string,
    dialect?: Dialect,
  ): Promise<PolyrhythmProfile> {
    return this.dispatchToWorker<PolyrhythmProfile>("analyzePolyrhythm", [
      text,
      dialect,
    ]);
  }

  async buildFullFingerprintAsync(
    text: string,
    dialect?: Dialect,
  ): Promise<FullPhonemicFingerprint> {
    return this.dispatchToWorker<FullPhonemicFingerprint>(
