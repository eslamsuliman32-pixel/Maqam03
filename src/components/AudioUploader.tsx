...
export const AudioUploader: React.FC<AudioUploaderProps> = ({
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "حدث خطأ غير متوقع أثناء تحليل الرابط. يرجى التأكد من صحة الرابط والمحاولة مرة أخرى.";
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      setMetadata(realAnalysis.metadata);

      onUploadComplete(realAnalysis);
      setStatus("COMPLETE");
    } catch (err) {
      console.error(err);
      const errorMessage =
        err instanceof Error
          ? err.message
          : "حدث خطأ غير متوقع أثناء معالجة الملف الصوتي. يرجى التأكد من أن الملف غير تالف والمحاولة مرة أخرى.";
      setError(errorMessage);
      setStatus("ERROR");
    }
  };
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="text-center">
              <h3 className="text-xl font-bold text-text-primary mb-2">
                اسحب وأفلت ملف الموسيقى
              </h3>
              <p className="text-sm text-text-muted">
                يدعم MP3, WAV, FLAC حتى 100MB
              </p>
            </div>
        ...
        {status === "ERROR" && (
          ...
          >
            ...
            <div className="text-center">
              <h3 className="text-lg font-bold text-quality-low mb-2">
                فشل التحميل
              </h3>
