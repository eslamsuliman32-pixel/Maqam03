...
import { MatchScoreBadge, SmartSuggestButton, TriggerAllSuggestionsButton } from "./SmartSuggestionsIntegrations";

// ────────────────────────────────────────────────────────────────────────

const BarTableRow = React.memo(
  ({
    // ... rest of BarTableRow remains same
    bar,
    isSelected,
    onToggleSelection,
    onFindSimilar,
    ...
    <tr
      className={`hover:bg-gold-400/5 transition-colors group ${isSelected ? "bg-gold-400/5" : ""}`}
    >
      ...
      <td className="px-3 py-2.5">
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex items-center gap-2 text-[10px] font-mono whitespace-nowrap">
            ...
            <div
              className="flex gap-0.5 items-end h-3 px-2 bg-bg-elevated rounded border border-border-default"
              dir="ltr"
            >
      ...
      <td className="px-3 py-2.5">
        <span className="px-2 py-0.5 bg-gold-400/10 text-gold-400 rounded text-[10px] font-bold">
          {moraEngine.extractCorePhoneme(bar.text) || bar.corePhoneme}
        </span>
      </td>
      <td className="px-3 py-2.5">
        <span className="text-[10px] font-mono text-text-secondary">
          {bar.emotion || "غير مصنف"}
        </span>
      </td>
      <td className="px-3 py-2.5">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5" title="الوزن الصوتي">
            ...
            <span className="text-[10px] font-mono text-quality-high font-bold">
              {bar.sonicWeight || 0}
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5" title="الوزن الإيقاعي">
            ...
            <span className="text-[10px] font-mono text-quality-medium font-bold">
      ...
      <td className="px-3 py-2.5 text-[10px] font-mono text-text-muted">
        <div className="flex flex-col gap-1">
          <span className="text-gold-400 font-bold uppercase">
            {bar.flowMode?.replace("_", " ")}
          </span>
          <span className="opacity-60">
            {moraEngine.getPhoneticTrait(bar.text)}
          </span>
        </div>
      </td>
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          <button
            onClick={() => {
              ...
              const event = new CustomEvent("export-to-workshop", {
                detail: { bar },
              });
          ...
          >
            <Star
              ...
              fill={bar.isFavorite ? "currentColor" : "none"}
            />
          </button>
          <button
...
import { applyDerivedFilters } from "../store/phonetic-filters";
...
export const BarRepositoryDisplay: React.FC<BarRepositoryDisplayProps> = ({
  ...
  const filteredBars = useMemo(() => {
    return applyDerivedFilters(bars, {
      searchQuery: storeSearchQuery || searchQuery,
      narrativeArcFilter,
      barRoleFilter,
      resonanceCharacterFilter,
      semanticTagFilter,
      ...
      dateRangeFilter,
      compositeFilter,
      phoneticFilter: phoneticFilter as any,
      passingThreshold
    });
  }, [
    bars,
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-bg-surface/30 p-6 rounded-3xl border border-border-default backdrop-blur-xl shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted mt-1 font-mono uppercase tracking-widest">
              إجمالي البارات المتاحة:{" "}
              <b className="text-gold-400">
                {bars.filter((b) => !b.deleted && b.text).length}
              </b>
            </p>
          </div>
          {selectedBars.length > 0 && (
            ...
            >
              ...
              <button
                ...
                className="px-3 py-1 bg-sky-500 text-bg-base rounded-lg text-[10px] font-bold hover:bg-sky-400 transition-all flex items-center gap-2"
              >
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 relative z-10 w-full lg:w-auto">
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                if (!isConfirmingClear) {
                  ...
                  console.log("Triggering clearRepository...");
                  clearRepository();
                  setIsConfirmingClear(false);
                }
              }}
              className={`px-4 py-2 rounded-xl transition-all flex items-center gap-2 text-[10px] font-bold shadow-lg ${
                isConfirmingClear 
                  ? "bg-red-500 text-white animate-pulse" 
                  : "bg-red-500/10 text-red-500 border border-red-500/30 hover:bg-red-500/20"
              }`}
              title="تفريغ المستودع بالكامل لمسح كافة البيانات"
            >
              <Trash2 className="w-3.5 h-3.5" />
              {isConfirmingClear ? "هل أنت متأكد؟ اضغط مرة أخرى" : "تفريغ المستودع"}
            </button>

            <button
              onClick={computeSignaturesOnly}
              disabled={storeIsLoading || isRefreshingRepo}
              className="px-4 py-2 bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 rounded-xl hover:bg-indigo-500/20 transition-all flex items-center gap-2 text-[10px] font-bold shadow-[0_0_15px_rgba(99,102,241,0.1)]"
              title="تحديث البصمات الرقمية للبارات فقط (أسرع من التحديث الشامل)"
            >
               {storeIsLoading ? (
                <div className="flex items-center gap-2">
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ...
            <button
              onClick={reprocessAllBars}
              disabled={storeIsLoading || isRefreshingRepo}
              className="px-4 py-2 bg-amber-500/10 text-amber-500 border border-amber-500/30 rounded-xl hover:bg-amber-500/20 transition-all flex items-center gap-2 text-[10px] font-bold shadow-[0_0_15px_rgba(245,158,11,0.1)]"
              title="إعادة معالجة شاملة لجميع البارات المحفوظة لضمان توافقها مع أحدث محركات التحليل"
            >
              {storeIsLoading ? (
                <div className="flex items-center gap-2">
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ...
            >
              <CloudDownload
                className={`w-3.5 h-3.5 ${isRefreshingRepo ? "animate-bounce" : ""}`}
              />
              استرجاع من السحابة
            </button>
            <TriggerAllSuggestionsButton bars={bars} />
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 py-2 px-1 border-b border-white/5">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 bg-bg-surface/40 border border-border-default rounded-xl px-3 py-1.5 backdrop-blur-sm">
            ...
            >
              ...
              <option value="moraCount">الطول (المقاطع)</option>
              <option value="sonicWeight">الوزن الصوتي</option>
            </select>
          </div>
          ...
          <div className="flex items-center gap-2 bg-bg-surface/40 border border-border-default rounded-xl px-3 py-1.5 backdrop-blur-sm">
            ...
            >
              ...
              <option value="ai">الموضوع</option>
              <option value="rhyme">القافية</option>
              <option value="family">المشاعر</option>
            </select>
          </div>
        ...
        <div className="flex items-center gap-4 relative">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 border rounded-2xl text-[10px] font-bold transition-all shadow-xl group ${isFilterOpen ? "bg-gold-400 text-bg-base border-gold-400" : "bg-white/5 border-white/10 text-text-muted hover:text-gold-400 hover:border-gold-400/30"}`}
          >
            <Filter className={`w-3.5 h-3.5 transition-transform duration-300 ${isFilterOpen ? "rotate-180" : ""}`} />
            فلترة متقدمة
          </button>
                          ...
                          <tr className="bg-bg-elevated/50 border-b border-border-default text-[10px] font-mono text-text-muted uppercase">
                            <th className="px-3 py-2.5 font-medium w-10">
                              <input
                                type="checkbox"
                                checked={
                                  selectedBars.length === barsInGroup.length &&
                                  barsInGroup.length > 0
                                }
                                onChange={() => {
                                  if (
                                    selectedBars.length === barsInGroup.length
                            ...
                            <th className="px-3 py-2.5 font-semibold text-center sticky left-0 bg-bg-elevated/95 backdrop-blur-md z-10 border-l border-border-default shadow-[-2px_0_5px_rgba(0,0,0,0.1)]">
                              إجراءات
                            </th>
                          </tr>
                        </thead>
                    ...
                    <AnimatePresence>
                      {barsInGroup.map((bar) => (
                        <BarCard
                          ...
                          bar={bar}
                          onDelete={(id) => {
                            const b = bars.find((item) => item.id === id);
                            if (b) handleDeleteRequest(b);
                          }}
                          onToggleFavorite={toggleFavorite}
                          onFindSimilar={handleFindSimilar}
                          isSelected={selectedBars.includes(bar.id)}
                          onToggleSelection={() => toggleSelection(bar.id)}
                        />
                      ))}
                    </AnimatePresence>
                  </div>
          ...
          {sortedBars.length === 0 && (
            ...
            >
              ...
              <p className="text-sm text-text-muted">
                جرب تغيير معايير البحث أو الفرز.
              </p>
            </motion.div>
          )}
        </div>
      </div>
      {/* Filter Panel (Replaced by AdvancedFilterPanel) */}
    </div>
  );
};
